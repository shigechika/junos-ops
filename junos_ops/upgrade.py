...
def get_disk_avail(hostname, dev) -> dict:
    """Return available disk space on the filesystem that contains ``rpath``.

    Calls ``get-system-storage-information`` and selects the mount point
    that is the longest prefix of ``rpath`` (most-specific match).
    ...
    try:
        xml = dev.rpc.get_system_storage_information()
        best = None
        best_len = -1
        for fs in xml.findall(".//filesystem"):
