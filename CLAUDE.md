...
- `get_interface_states(dev, names)` — `get-interface-information(terse=True)` を 1 回で引き、`{name: "admin/oper"}`（terse の text ノードは前後に改行が入るので `strip()` 必須）。未報告は None
- `_poll_device(hostname, timeout, interval, check)` — 再接続ポーリングの共通実装（probe は残り時間で上限、`time.sleep`/`monotonic` はモジュール属性経由）。`wait_for_master` と `wait_for_member` が共用
- `wait_for_member(hostname, member, timeout, interval=15, expect_up=())` — member の復帰待ち。条件は「VC 上で `Prsnt` かつ role がある」＋「FPC スロットが `Online`」＋（指定時）`expect_up` の全ポートが `up/up`。`Prsnt` だけでは PFE 未準備でブラックホールになりうるため FPC とポートまで見る。接続失敗は「まだ」（member の uplink を管理経路が通ると VC 全体が一時的に不達になる）
...
- `cmd_reboot(hostname)` — `upgrade.reboot` → 接続を閉じる → `--member --now` かつ `--wait > 0` かつ `code==0` かつ dry-run でないときだけ `vc.wait_for_member` を呼び、結果に `wait`/`after`/`fpc_state`/`interfaces` をマージ。戻らなければ `code=10`・`error=member_not_back`（`--at` のスケジュール実行や dry-run は検証しない）
