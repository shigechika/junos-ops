...
- `get_fpc_state(dev, slot)` — `get-fpc-information` からスロットの `state`（`Online`/`Empty`/…）。取得不能は None
- `get_interface_states(dev, names)` — `get-interface-information(terse=True)` を 1 回で引き、`{name: "admin/oper"}`。**physical / logical 両方の `name` を見る**ので `ge-0/0/40` も `ae0.0` も指定可（terse の text ノードは前後に改行が入るので `strip()` 必須）。未報告は None
- `_poll_device(hostname, timeout, interval, check, on_unreachable=None)` — 再接続ポーリングの共通実装（probe の `auto_probe` と **`dev.timeout` の両方**を上限（後者は `interval` 側で頭打ち — RPC 1 本で予算を食い潰させない）。`--wait` は probe 間で判定する予算であってハードキルではない。`time.sleep`/`monotonic` はモジュール属性経由）。接続失敗ごとに `on_unreachable()` を呼ぶ
- `get_member_boot_time(dev, member, master=None)` — `show system uptime` の member ブロック（`fpcN`、master なら `localre`）の `System booted` 文字列（解釈するのは `boot_time_is_newer()` だけ）。**per-RE ブロックを持たない応答はセッションのいる RE の値でしかない**ので、`master` が分かっていて member と一致しないときは None を返す（他 member の boot 時刻をベースラインにすると、いつまでも変化せず必ず timeout する）
- `wait_for_member(hostname, member, timeout, interval=15, expect_up=(), booted_before=None, master=None)` — member の復帰待ち。条件は「VC 上で `Prsnt` かつ role がある」＋「FPC スロットが `Online`」＋**「再起動したという証拠」**＋（指定時）`expect_up` の全ポートが `up/up`。`Prsnt` だけでは PFE 未準備でブラックホールになりうるため FPC とポートまで見る。**証拠が要る理由**: reboot RPC は member が落ちる前に戻るので、最初の probe は「再起動前の健全な member」を見て誤って成功と判定しうる（codex 指摘、#164）。`booted_before`（cmd_reboot が発行**前**に読む）より**新しければ**再起動済み（`boot_time_is_newer()` が両方 parse できれば instant 比較、できなければ文字列不一致 — 同一 boot の再フォーマット/別 TZ 表記を誤検知しない）。boot 時刻を引く member ブロックは **その probe 時点の master** で決める（待機中に mastership が動くと localre/fpcN の対応が変わる）、読めなかった場合は「一度は不達 or member が absent/not Prsnt/FPC not Online を観測した」ことを条件にする（`_poll_device` の `on_unreachable` コールバックで接続失敗も transition として数える。**VC status の RPC 失敗も FPC state 不明（`get_fpc_state` が None）も member について何も語らないので transition に数えない**）。接続失敗は「まだ」（member の uplink を管理経路が通ると VC 全体が一時的に不達になる）
...
- `cmd_reboot(hostname)` — `--member --now --wait` 時は**発行前に**同じ接続で `vc.get_vc_status` と `vc.get_member_boot_time` を読んでベースラインを取る → `upgrade.reboot` → 接続を閉じる → `code==0` かつ dry-run でないときだけ `vc.wait_for_member(..., booted_before=, master=)` を呼び、結果に `wait`/`after`/`fpc_state`/`interfaces` をマージ。戻らなければ `code=10`・`error=member_not_back`（`--at` のスケジュール実行や dry-run は検証しない）
- `cmd_vc_switch(hostname)` — `vc.master_switch` → 接続を閉じる → `--wait > 0` かつ（`initiated_unverified` **または** `rejected` かつ `issued`）なら `vc.wait_for_master(hostname, expected_master, wait)` を結果にマージ（`confirmed`/`verification_failed`、`after`/`after_replication`/`wait`）。**`rejected` でも発行済みなら実機の状態を見る**（応答テキストは成功バナーと診断が混在しうるため断定しない。mastership が動いていれば `confirmed` に格上げするが**因果は主張しない** — 並行/手動切替でも同じ状態になりうるため、拒否に読めた応答を `rejected_reply` に保存し「このコマンドが動かしたとは限らない」と警告する。動いていなければ `rejected` のまま「拒否は本物」と記録）。`issued=False`（事前確認拒否・両形式 not valid）は検証しない。終了コードは `ok` ベース（`confirmed`/`dry_run`/`initiated_unverified`(--wait 0) → 0）。`_run` のガード: ホスト名必須・`--wait >= 0`
...
junos-ops reboot --member N (--now | --at YYMMDDHHMM) [--allow-mixed-version] [--wait SEC] [--expect-up IFACE,...] hostname ...  # VC member 個別リブート（ホスト名必須。--wait で復帰検証）
