...
- `_poll_device(hostname, timeout, interval, check, on_unreachable=None)` — 再接続ポーリングの共通実装（probe の `auto_probe` と **`dev.timeout` の両方**を残り時間で上限 — RPC 側を縛らないと `--wait` を超過する。`time.sleep`/`monotonic` はモジュール属性経由）。接続失敗ごとに `on_unreachable()` を呼ぶ
...
- `wait_for_member(hostname, member, timeout, interval=15, expect_up=(), booted_before=None, master=None)` — member の復帰待ち。条件は「VC 上で `Prsnt` かつ role がある」＋「FPC スロットが `Online`」＋**「再起動したという証拠」**＋（指定時）`expect_up` の全ポートが `up/up`。`Prsnt` だけでは PFE 未準備でブラックホールになりうるため FPC とポートまで見る。**証拠が要る理由**: reboot RPC は member が落ちる前に戻るので、最初の probe は「再起動前の健全な member」を見て誤って成功と判定しうる（codex 指摘、#164）。`booted_before`（cmd_reboot が発行**前**に読む）と違っていれば再起動済み、読めなかった場合は「一度は不達 or not ready を観測した」ことを条件にする（`_poll_device` の `on_unreachable` コールバックで接続失敗も transition として数える）。接続失敗は「まだ」（member の uplink を管理経路が通ると VC 全体が一時的に不達になる）
